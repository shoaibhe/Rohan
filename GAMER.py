...
# Global variables
KEY_PRICES = {
    'hour': 80,  # 80 Rs per hour
    'day': 150,   # 150 Rs per day
    'week': 650, # 650 Rs per week
...
last_attack_times = {}
COOLDOWN_MINUTES = 0  # Cooldown permanently disabled
attack_in_progress = False  # Track if an attack is running
ALLOWED_GROUP_ID = -1002328886935  # Replace with your actual group ID
...
def check_cooldown(user_id: int) -> Tuple[bool, int]:
    ...
    """
    return False, 0  # Cooldown disabled, always allow attack
...
def set_thread_count(message):
    ...
    if not is_super_admin(user_id):
        bot.send_message(chat_id, "*🤡🤡You are not authorized to change thread settings pls context owner @Rohan2349.🤬🤬*", parse_mode='Markdown')
        return
...
def set_packet_size(message):
    ...
    # Check if the message is from the allowed group
    if not is_allowed_group(chat_id):
        bot.send_message(chat_id, "🤬⚠️This bot can only be used in the specific group. Please use it there.🤬")
        return
    ...
    if not is_super_admin(user_id):
        bot.send_message(chat_id, "*🤡🤡You are not authorized to change packet size settings pls contect from @Rohan2349.*🤬🤬", parse_mode='Markdown')
        return
...
def attack_command(message):
    ...
    # Cooldown check removed - proceed directly to attack logic
...
def process_attack_command(message, chat_id):
    try:
        ...
        # Cooldown check removed - proceed directly to attack processing
...
def send_owner_info(message):
    ...
    owner_message = "💀💀This Bot Has Been Developed By ᚛ @sadiq12312 ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"💀💀Welcome, Super Admin! Developed By ᚛ @sadiq12312 ᚜\n\n"
            f"🥵Admin Commands:\n"
            ...
            f"☠️/packet - Set packet size\n"
        )
    elif is_admin(user_id):
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"☠️☠️Welcome, Admin! Developed By ᚛ @sadiq12312 ᚜\n\n"
...
def genkey_command(message):
    ...
    if len(cmd_parts) != 3:
        bot.send_message(chat_id, (
            ...
            "- hour/hours (60₹ per hour)\n"
            "- day/days (150₹ per day)\n"
            "- week/weeks (650₹ per week)"
            "- Month/Months (1200₹ per Month)\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*😭😭𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Rohan2349 or @sadiq12312 ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*😭𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Rohan2349 or @sadiq12312 ᚜.😭*", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                "😭😭𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞�{d𝐞𝐞𝐦 𝐚 𝐧𝐞𝐰 𝐤𝐞𝐲.\n"
                "Contact ᚛ @Rohan2349 or @sadiq12312 ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "😭😭𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Rohan2349 or @sadiq12312 ᚜."
